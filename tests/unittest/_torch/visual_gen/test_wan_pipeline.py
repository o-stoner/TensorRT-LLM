...
"""Integration tests for Wan T2V pipeline.
...
_DEFAULT_T2V_CHECKPOINT = "/home/scratch.trt_llm_data_ci/llm-models/Wan2.2-T2V-A14B-Diffusers"


def _resolve_checkpoint_t2v() -> str:
    """
    ...
    but its model_index.json identifies it as an I2V pipeline.
    """
    path = Path(os.environ.get("DIFFUSION_MODEL_PATH", _DEFAULT_T2V_CHECKPOINT))
    ...
    if model_index.exists():
        ...
        if "ImageToVideo" in class_name or "i2v" in class_name.lower():
            raise ValueError(
                f"Checkpoint at {path} appears to be a Wan I2V model, not T2V "
                f"(model_index.json _class_name={class_name!r}). "
                "Set DIFFUSION_MODEL_PATH to a Wan T2V checkpoint."
            )

    return str(path)


CHECKPOINT_PATH = _resolve_checkpoint_t2v()

SKIP_COMPONENTS = []


def _get_wan_transformer_inputs(transformer, device="cuda", dtype=torch.bfloat16):
    """Create test inputs for the Wan transformer (H=W=64, T=1, seq_len=32)."""
...
def checkpoint_exists():
    """Skip test if T2V checkpoint is unavailable."""
    if not CHECKPOINT_PATH or not os.path.exists(CHECKPOINT_PATH):
        pytest.skip(
            f"Wan T2V checkpoint not found at {CHECKPOINT_PATH}. "
            "Set DIFFUSION_MODEL_PATH to override."
        )
    return True


def _load_t2v_pipeline(quant_config=None):
    """Load Wan T2V pipeline inline. Caller must delete and empty_cache."""
    args = DiffusionArgs(
...
class TestWanPipelineLoading:
    """Integration tests for Wan T2V pipeline loading."""
    ...
    def _make_tiny_model_config(cls, boundary_ratio=None) -> DiffusionModelConfig:
        config_dict = dict(cls._TINY_T2V_CONFIG)
    ...
    def test_wan21_t2v_instantiation(self):
        """Test Wan 2.1 T2V pipeline (single-stage) instantiates correctly."""
        from tensorrt_llm._torch.visual_gen.models.wan.pipeline_wan import WanPipeline

        pipeline = WanPipeline(self._make_tiny_model_config(boundary_ratio=None))
        assert pipeline.transformer is not None
        assert pipeline.transformer_2 is None
        assert pipeline.boundary_ratio is None

    def test_wan22_t2v_instantiation(self):
        """Test Wan 2.2 T2V pipeline (two-stage) instantiates correctly."""
        from tensorrt_llm._torch.visual_gen.models.wan.pipeline_wan import WanPipeline

        pipeline = WanPipeline(self._make_tiny_model_config(boundary_ratio=0.4))
        assert pipeline.transformer is not None
    ...
    def test_attention_backend_config(self, backend: str):
        ...
        model_config = DiffusionModelConfig(
            pretrained_config=SimpleNamespace(**self._TINY_T2V_CONFIG),
            quant_config=QuantConfig(),
...
class TestWanE2EvsHF:
    """Compare TRT-LLM Wan T2V output against HuggingFace reference."""

    _PROMPT = "A serene mountain lake at sunrise"
    # Explicit negative prompt passed to both pipelines
    _NEGATIVE_PROMPT = ""
    _HEIGHT = 480
    _WIDTH = 832
    _NUM_FRAMES = 5
    ...
    def test_e2e_vs_hf(self, checkpoint_exists):
        """TRT-LLM T2V pipeline output must closely match HuggingFace reference.

        Runs both pipelines sequentially (HF then TRT-LLM) at 480x832 / 5 frames /
        5 steps with the same seed, then asserts cosine similarity > 0.99.
        """
        pytest.importorskip("ftfy", reason="ftfy required by diffusers WanPipeline prompt cleaning")
        try:
            from diffusers import WanPipeline as HFWanPipeline
        except ImportError:
            pytest.skip("diffusers WanPipeline not available")

        # --- 1. HF pipeline ---
        gc.collect()
        torch.cuda.empty_cache()
        hf_pipe = HFWanPipeline.from_pretrained(CHECKPOINT_PATH, torch_dtype=torch.bfloat16)
        hf_pipe.to("cuda")
        ...
        assert cos_sim > 0.99, (
            f"TRT-LLM T2V output deviates too much from HF reference: cos_sim={cos_sim:.6f} < 0.99"
        )
...
def fp8_refs():
    ...
    pipeline_bf16 = _load_t2v_pipeline()
    try:
        ...
        # Full E2E reference
        e2e_inputs = _get_wan_transformer_inputs(transformer_bf16)
        with torch.no_grad():
    ...
    torch.cuda.reset_peak_memory_stats()
    pipeline_fp8 = _load_t2v_pipeline(quant_config={"quant_algo": "FP8", "dynamic": True})
...
class TestWanFP8:
    """FP8 quantization, numerical correctness, and memory tests for Wan T2V."""
    ...
    def test_fp8_vs_bf16_full_transformer_e2e(self, fp8_refs):
        """End-to-end test: Compare full Wan T2V transformer FP8 vs BF16 output."""
        inputs_fp8 = _get_wan_transformer_inputs(fp8_refs.pipeline_fp8.transformer)
        with torch.no_grad():
